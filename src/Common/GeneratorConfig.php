...
namespace Number7even\Generator\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsDataTables = config('Number7even.laravel_generator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'Number7even.laravel_generator.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
            'Number7even.laravel_generator.namespace.api_controller',
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'Number7even.laravel_generator.path.repository',
        ...
        $this->pathDataTables = config('Number7even.laravel_generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'Number7even.laravel_generator.path.api_controller',
        ...
        $this->pathApiTestTraits = config('Number7even.laravel_generator.path.test_trait', base_path('tests/traits/'));

        $this->pathController = config(
            'Number7even.laravel_generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('Number7even.laravel_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('Number7even.laravel_generator.path.routes', app_path('Http/routes.php'));

        $this->pathViews = config(
            'Number7even.laravel_generator.path.views',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('Number7even.laravel_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('Number7even.laravel_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        $this->options['softDelete'] = config('Number7even.laravel_generator.options.softDelete', false);
    ...
    {
        $this->prefixes['route'] = explode('/', config('Number7even.laravel_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('Number7even.laravel_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('Number7even.laravel_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('Number7even.laravel_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('Number7even.laravel_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('Number7even.laravel_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('Number7even.laravel_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('Number7even.laravel_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('Number7even.laravel_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
