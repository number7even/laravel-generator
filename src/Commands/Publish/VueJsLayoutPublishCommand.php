...
class VueJsLayoutPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'Number7even.publish:vuejs';
    ...
    {
        $viewsPath = config('Number7even.laravel_generator.path.views', base_path('resources/views/'));
        $resourcesPath = config('Number7even.laravel_generator.path.resourcesPath', base_path('resources/'));
        $vendorPath = $resourcesPath.'assets/vendor/';
        $assetsJsPath = config('Number7even.laravel_generator.path.assetsJsPath', base_path('resources/assets/js/'));
        $assetsCssPath = config('Number7even.laravel_generator.path.assetsCssPath', base_path('resources/assets/css/'));
        $templateType = config('Number7even.laravel_generator.templates', 'core-templates');
        $requestPath = config('Number7even.laravel_generator.path.api_request', base_path('app/Http/Requests/'));
        ...
        foreach ($files as $stub => $blade) {
            $sourceFile = base_path('vendor/Number7evenlabs/'.$templateType.'/templates/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($vueLayoutFiles as $stub => $blade) {
            $sourceFile = base_path('vendor/Number7evenlabs/'.$templateType.'/templates/vuejs/'.$stub);
            $destinationFile = $viewsPath.$blade;
            $this->publishFile($sourceFile, $destinationFile, $blade);
        }

        foreach ($baseRequestCustomFiles as $stub => $php) {
            $sourceFile = base_path('vendor/Number7evenlabs/laravel-generator/templates/'.$stub);
        ...
        $sourceFile = base_path('vendor/Number7evenlabs/laravel-generator/templates/vuejs/js/gulpfile.js');
        $destinationFile = base_path().'/gulpfile.js';
        $this->publishFile($sourceFile, $destinationFile, 'gulpfile.js');

        $sourceFile = base_path('vendor/Number7evenlabs/laravel-generator/templates/vuejs/js/package.json');
    ...
    {
        $path = config('Number7even.laravel_generator.path.routes', app_path('Http/routes.php'));
    ...
    private function publishHomeController()
    {
        ...
        $controllerPath = config('Number7even.laravel_generator.path.controller', app_path('Http/Controllers/'));
    ...
    private function fillTemplate($templateData)
    {
        $templateData = str_replace(
            ...
            config('Number7even.laravel_generator.namespace.controller'), $templateData
        );

        $templateData = str_replace(
            '$NAMESPACE_REQUEST$',
            config('Number7even.laravel_generator.namespace.request'), $templateData
        );
